/**
 * ActionUIScrollableArea - A comprehensive, reusable scrollable area component for ActionEngine
 *
 ...
 * this.inventoryScroller = new ActionUIScrollableArea({
...
class ActionUIScrollableArea {
     ...
     * const inventoryScroller = new ActionUIScrollableArea({
     ...
     * const customScroller = new ActionUIScrollableArea({
     *     listAreaX: 400, listAreaY: 100, listAreaWidth: 450, listAreaHeight: 400,
     ...
     * const chatScroller = new ActionUIScrollableArea({
     *     listAreaX: 50, listAreaY: 100, listAreaWidth: 300, listAreaHeight: 400,
    ...
    handleInput(deltaTime) {
        ...
        const isDown = this.input.isPointerDown ? this.input.isPointerDown() : this.input.isLeftMouseButtonDown();
        const justUp = !isDown && this._prevPointerDown;
        ...
        // Forward item clicks to onItemClick callback
        if (this.onItemClick && justUp && this._lastItems) {
            for (let i = 0; i < this._lastItems.length; i++) {
                const itemBounds = this.getItemBounds(i);
                if (pointer.x >= itemBounds.x && pointer.x <= itemBounds.x + itemBounds.width &&
                    pointer.y >= itemBounds.y && pointer.y <= itemBounds.y + itemBounds.height) {
                    this.onItemClick(this._lastItems[i], i, pointer);
                    break;
                }
            }
        }
     ...
     *
     * @example
     * this.roomScroller = new ActionUIScrollableArea({
     *     enableClipping: true,
     ...
     *
     * Similar to scrollToSelectedItem() in ActionNetManagerGUI.
     * Calculates the item's Y position and adjusts scrollOffset to keep it visible.
     *
     * @param {number} index - Zero-based index of the item to scroll into view
     *
     * @example
     * // After navigating to item 10 via keyboard, ensure it's visible
     * scroller.scrollItemIntoView(10);
     */
    scrollItemIntoView(index) {
        if (index < 0 || index >= this._lastItems?.length) {
            return; // Invalid index
        }

        // Calculate the Y position of this item (relative to list area top)
        const itemHeight = this.listArea.itemHeight + this.listArea.padding;
        const itemY = index * itemHeight;

        // Calculate visible area bounds
        const scrollTop = this.scrollOffset;
        const scrollBottom = scrollTop + this.listArea.height;
        ...
        // Check if item is below visible area (scroll down)
            ...
            this.scrollOffset = itemY + itemHeight - this.listArea.height;
        }
