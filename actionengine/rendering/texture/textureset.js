...
class TextureSet {
    ...
    async _createTextureArray(images) {
        ...
        const { width, height, effectiveCount } = this._calculateOptimalTextureSize(images, gl);
        ...
        const maxUpload = Math.min(images.length, effectiveCount);
     ...
     * @private
     */
    _calculateOptimalTextureSize(images, gl) {
        let maxDim = 0;
        let validCount = 0;
        ...
        if (validCount === 0) validCount = images.length;
        ...
        console.log(
            `[TextureSet] Array: ${effectiveCount} layers at ${dim}x${dim}` +
