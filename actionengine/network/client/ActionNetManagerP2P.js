...
class ActionNetManagerP2P {
    ...
    generateUniqueDisplayName(username, excludeId = null) {
        const allDisplayNames = this.connectedUsers.filter((u) => u.id !== excludeId).map((u) => u.displayName);

        const countMap = {};

        // Count existing instances of this username (for display name generation)
        const allUsernames = this.connectedUsers.filter((u) => u.id !== excludeId).map((u) => u.username);
    ...
    setUsername(name) {
        ...
        if (!/^[a-zA-Z0-9_'-]+$/.test(name)) {
            return Promise.reject(
                new Error("Username can only contain letters, numbers, underscores, hyphens, and apostrophes")
            );
        }
