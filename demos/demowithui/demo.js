...
class Game {
	...
	constructor(canvases, input, audio) {
		...
		// Initialize ActionUI system with theme
		this._setupActionUI();
	...
	/**
	 * Setup ActionUI components to replace custom canvas-based UI
	 */
	_setupActionUI() {
		// Create ActionUI instance
		const canvases = {
		...
		// Apply custom theme colors matching the demo's color scheme
		this.ui.setThemeOverride({
			colorBackground: '#0d0d1a',
			colorSurface: '#131326',
			colorSurfaceRaised: '#1a1a3a',
			colorPrimary: '#00f0f0',
			colorPrimaryHover: '#00d0d0',
			colorAccent: '#00e5cc',
			colorSuccess: '#00c896',
			colorDanger: '#ff6b6b',
			colorWarning: '#ffb700'
		...
		// Build main control panel with buttons
		this._buildMainPanel();

		// Build stats panel
		this._buildStatsPanel();

		// Build debug panel (for F9 overlay)
		this._buildDebugPanel();
	}

	/**
	 * Build the main control panel with interactive buttons
	 */
	_buildMainPanel() {
		...
		// Button 1 - Toggle inactive button
		this.ui_button1 = new ActionUIButton({
			x: 10 + padding,
			y: cy,
			width: btnWidth,
			height: btnHeight,
			...
			variant: 'primary',
			onClick: () => {
				// Toggle the 4th button's enabled state
				this.ui_inactiveButton.enabled = !this.ui_inactiveButton.enabled;
				this.ui_inactiveButton.text = this.ui_inactiveButton.enabled ? 'Active Button' : 'Inactive Button';
				this.addMessage(`Inactive button ${this.ui_inactiveButton.enabled ? "enabled" : "disabled"}`);
				this.totalClicks++;
				this.audio.play("sound5", { volume: 0.7 });
			}
		});
		panel.addChild(this.ui_button1);
		cy += 40;

		// Button 2
		this.ui_button2 = new ActionUIButton({
			x: 10 + padding,
			y: cy,
			width: btnWidth,
			height: btnHeight,
			...
			onClick: () => {
				...
				this.audio.setSoundVolume("sound5", 0.7);
				this.audio.play("sound5");
				this.totalClicks++;
			}
		});
		panel.addChild(this.ui_button2);
		cy += 40;

		// Button 3
		this.ui_button3 = new ActionUIButton({
			x: 10 + padding,
			y: cy,
			width: btnWidth,
			height: btnHeight,
			...
			variant: 'success',
			onClick: () => {
				this.addMessage("Button 3 was pressed!");
				this.audio.play("sound6", { volume: 0.6 });
				this.totalClicks++;
			}
		});
		panel.addChild(this.ui_button3);
		cy += 40;

		// Inactive button (can be toggled by button1)
		this.ui_inactiveButton = new ActionUIButton({
			x: 10 + padding,
			y: cy,
			width: btnWidth,
			height: btnHeight,
			...
			enabled: false,
			onClick: () => {
				if (!this.ui_inactiveButton.enabled) {
					this.addMessage("Button is inactive - click Click Me 1 to activate it!");
					return;
				}
		...
		panel.addChild(this.ui_inactiveButton);
		cy += 40;

		// Spawn character button
		this.ui_spawnButton = new ActionUIButton({
			x: 10 + padding,
			y: cy,
			width: btnWidth,
			height: btnHeight,
		...
		panel.addChild(this.ui_spawnButton);
	}
	...
	_buildStatsPanel() {
		...
		// Total clicks label
		this.ui_statsLabel = new ActionUILabel({
			x: 590 + padding,
			y: cy,
			width: 400,
			height: 20,
			text: `Total Clicks: ${this.totalClicks}`,
			fontSize: 16,
			fontWeight: 'normal',
			color: '#00f0f0'
		});
		panel.addChild(this.ui_statsLabel);
		cy += 15;

		// Objects in world label
		this.ui_objectsLabel = new ActionUILabel({
			x: 590 + padding,
			y: cy,
			width: 400,
			height: 20,
			text: `Objects in World: ${this.physicsWorld.objects.size}`,
			fontSize: 16,
			fontWeight: 'normal',
			color: '#00f0f0'
		});
		panel.addChild(this.ui_objectsLabel);
		cy += 15;

		// Character state label
		this.ui_charStateLabel = new ActionUILabel({
			x: 590 + padding,
			y: cy,
			width: 400,
			height: 20,
			text: this.player ? `Character: ${this.player.debugInfo?.state?.current || 'unknown'}` : 'Character: Not spawned',
			fontSize: 16,
			fontWeight: 'normal',
			color: '#00f0f0'
		});
		panel.addChild(this.ui_charStateLabel);
	}

	/**
	 * Build the debug console panel on the debug layer
	 */
	_buildDebugPanel() {
		const panel = new ActionUIPanel({
			x: 10,
			...
			width: 780,
			height: 540,
			title: 'Debug Console (F9)',
			shadow: true,
			visible: false,
			layer: 'debug',
			opacity: 0.75  // Semi-transparent to see the game through it
		});
		this.ui.add(panel);
		this.debugPanel = panel;

		// Message ListView
		this.debugListView = new ActionUIListView({
			x: 20,
			y: 50,
			width: 760,
			height: 430,
		...
		panel.addChild(this.debugListView);

		// Text input for debug console
		this.debugConsoleInput = new ActionUITextInput({
			x: 20,
			y: 500,
			width: 650,
			height: 32,
			...
			onSubmit: (value) => {
				if (value.trim()) {
					...
					this.debugConsoleInput.value = '';
					// Add to ListView as string
					this.debugListView.addItem(this.messages[0]);
				}
			}
		});
		panel.addChild(this.debugConsoleInput);

		// Hide/Show GUI button
		this.hideGUIButton = new ActionUIButton({
			x: 680,
			y: 500,
			width: 100,
			height: 32,
			...
			layer: 'debug',
			onClick: () => {
				this.guiCanvas.style.display = this.guiCanvas.style.display === 'none' ? 'block' : 'none';
				this.hideGUIButton.text = this.guiCanvas.style.display === 'none' ? 'Show GUI' : 'Hide GUI';
				this.addMessage('GUI toggled');
			}
		});
		panel.addChild(this.hideGUIButton);
	}
	...
	addMessage(msg) {
		...
		// Update listview in real-time when debug panel is visible
		if (this.debugListView && this.showDebug) {
			this.debugListView.addItem(timestampedMsg);
		}
	}
	...
	spawnCharacter() {
		...
		if (!this.player) {
			...
			// Disable UI keyboard navigation when character is spawned (let game keys take priority)
	 ...
	 */
	update(deltaTime) {
		// Update ActionUI components (buttons, panels, inputs, etc.)
		...
		this.ui_statsLabel.text = `Total Clicks: ${this.totalClicks}`;
		this.ui_objectsLabel.text = `Objects in World: ${this.physicsWorld.objects.size}`;
		this.ui_charStateLabel.text = this.player ? `Character: ${this.player.debugInfo?.state?.current || 'unknown'}` : 'Character: Not spawned';
	}
	...
	handleCommonInput() {
		...
		if (this.input.isKeyJustPressed("ActionDebugToggle")) {
			...
			this.debugPanel.visible = this.showDebug;
			
			if (this.showDebug) {
				// Hide main panel and enable keyboard navigation in debug mode
				this.ui.get(this.ui_button1.id)._parent.visible = false;
				this.ui.enableKeyboardNavigation = true;
				
				// Populate list with all current messages
				this.debugListView.clear();
				this.messages.forEach(msg => this.debugListView.addItem(msg));
			} else {
				// Show main panel again
				this.ui.get(this.ui_button1.id)._parent.visible = true;
				
				console.log("Closing debug. debugConsoleInput._parent:", this.debugConsoleInput._parent?.id, "panel visible:", this.debugPanel.visible);
				
				this.debugConsoleInput._focused = false;
				this.ui._focusedId = null;
				this.debugCtx.clearRect(0, 0, Game.WIDTH, Game.HEIGHT);
				
				// Disable keyboard navigation when exiting debug if character is spawned
				if (this.player) {
					this.ui.enableKeyboardNavigation = false;
				}
			}
	...
	draw() {
		 ...
		 * - Always 2D for crisp UI rendering with ActionUI components
		 * - Buttons, menus, HUD elements rendered by ActionUI
		 * - Stays pixel-perfect regardless of game context
		 */
		// Clear GUI canvas first
		this.guiCtx.clearRect(0, 0, this.guiCanvas.width, this.guiCanvas.height);
		// Draw ActionUI components
		this.ui.draw();
	...
	drawInstructions() {
		...
		this.guiCtx.fillText("ActionEngine Demo", 10, 3);

		if (this.player) {
			this.guiCtx.fillText("Action1: Jump (when character is on ground)", 10, 420);
			this.guiCtx.fillText("DirUp/Down/Left/Right: Move character", 10, 440);
			this.guiCtx.fillText("F9: Toggle debug overlay (switches to 2D mini-game)", 10, 460);
		} else {
			this.guiCtx.fillText("Use Arrow Keys to move the free camera", 10, 420);
			this.guiCtx.fillText("Click the 'Spawn Character' button to create a playable character", 10, 440);
			this.guiCtx.fillText("F9: Toggle debug overlay (switches to 2D mini-game)", 10, 460);
		}

		this.guiCtx.fillText("Action2: Reset all physics objects", 10, 480);
		this.guiCtx.fillText("Action3: Spawn random colorful physics objects (spheres, boxes, capsules!)", 10, 500);
		this.guiCtx.fillText(`Objects in world: ${this.physicsWorld.objects.size}`, 10, 520);

		// Display character debug info if available
		if (this.player && this.player.debugInfo) {
			const debugInfo = this.player.debugInfo;
			this.guiCtx.fillText(`Character state: ${debugInfo.state.current}`, 10, 540);
	...
	drawDebugLayer() {
		...
		// Render ActionUI debug components (panel, listview, text input)
		this.ui.draw('debug');
	}
