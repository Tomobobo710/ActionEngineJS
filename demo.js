...
class Game {
	...
	static DEMO_RENDER_MODE = "3d"; // Try changing this to '2d'!
	...
	initGL() {
       ...
       `;
		...
		const pixels = new Uint8Array([255, 255, 255, 255, 0, 0, 0, 255, 0, 0, 0, 255, 255, 255, 255, 255]);
		const level = 0;
	...
	update() {
		...
		const isInBounds = this.input.isPointInBounds(
			// Track pointer intersection with the UI element 'button1'
			mousePos.x,
			mousePos.y,
			{
				x: this.button1.x,
		...
		if (this.button1.hovered && this.input.isPointerDown()) {
			// Combine hover and click states to achieve a similar result to isElementPressed()
	...
	drawCube() {
		...
		Matrix4.translate(modelViewMatrix, modelViewMatrix, [-0.0, 0.0, -6.0]);

		Matrix4.rotate(modelViewMatrix, modelViewMatrix, this.rotationX, [1, 0, 0]);

		Matrix4.rotate(
			modelViewMatrix, // Matrix4 has all these methods, and many other commonly utilized methods for game development
			modelViewMatrix,
			this.rotationY,
			[0, 1, 0]
		);
	...
	drawScene3D() {
		...
		const modelViewMatrix = Matrix4.create();
		Matrix4.translate(modelViewMatrix, modelViewMatrix, [this.position.x, this.position.y, this.position.z]);
		Matrix4.rotate(modelViewMatrix, modelViewMatrix, this.rotationX, [1, 0, 0]);
	...
	drawScene2D() {
		...
		const gradient = this.ctx.createRadialGradient(-5, -5, 1, -5, -5, this.ballRadius * 2);
		...
		for (let i = 0; i < segments; i++) {
			...
			this.ctx.beginPath();
			this.ctx.moveTo(0, 0);
			this.ctx.lineTo(Math.cos(angle) * this.ballRadius, Math.sin(angle) * this.ballRadius);
			this.ctx.lineTo(
	...
	drawGameElement2D() {
		this.ctx.fillStyle = this.input.isElementHovered("gameElement", "game") ? "#66ff66" : this.gameElement.color;

		this.ctx.fillRect(this.gameElement.x, this.gameElement.y, this.gameElement.width, this.gameElement.height);
