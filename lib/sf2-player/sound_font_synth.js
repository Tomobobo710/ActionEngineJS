...
class Synthesizer {
  ...
  constructor (input) {
    ...
    // Then in the constructor, replace the reverb creation loop with:
if (!Synthesizer.sharedReverb) {
    Synthesizer.sharedReverb = new Reverb(this.ctx, { mix: 0.315 });
}
this.reverb = new Array(16).fill(Synthesizer.sharedReverb);
  }
  ...
  noteOn(channel, key, velocity) {
    ...
    let bank = (typeof this.bankSet[bankIndex] === 'object') ? this.bankSet[bankIndex] : this.bankSet[0];
    let instrument;

    // Add safety checks
    if (!bank) {
        console.warn(`Bank ${bankIndex} not found, falling back to bank 0`);
        bank = this.bankSet[0];
        if (!bank) {
            console.error('No valid banks found in soundfont');
            return;
        }
    }
    ...
    // Add safety check for instrument
    if (!instrument) {
        console.warn(`No valid instrument found for channel ${channel}, program ${this.channelInstrument[channel]}`);
        return;
    }
