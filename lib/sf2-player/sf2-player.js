...
class SoundFont {
  constructor(ctx) {  // Add ctx parameter
        this.ctx = ctx;
  ...
  async bootSynth(arrayBuffer) {
        ...
        // Pass the context to Synthesizer
        this.synth = new Synthesizer(input, this.ctx);  // Pass ctx here
        this.synth.init();
        this.synth.start();
  ...
  async loadSoundFontFromBase64(base64String) {
    const base64 = base64String.split(',')[1]; // Strip the "data:" prefix if present
