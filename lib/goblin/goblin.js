...
Goblin.CharacterController.prototype.updateGroundSpring = function () {
    ...
    var rayEnd = new Goblin.Vector3(rayStart.x, rayStart.y - this.ray_length, rayStart.z);
    ...
    if (intersections.length > 0) {
        ...
        this._lastHit = hit;
        this._lastGroundObject = hit.object;
        ...
        var smoothedForce =
            this._last_spring_force + (rawSpringForce - this._last_spring_force) * (1 - this.force_smoothing);
        ...
        if (hit.object && hit.object._mass !== Infinity) {
            hit.object.applyForceAtWorldPoint(new Goblin.Vector3(0, -smoothedForce, 0), hit.point);
        }
...
Goblin.CharacterController.prototype.getDebugInfo = function () {
    ...
    var projectedMovement = null;
    if (this._projected_movement) {
        projectedMovement = {
    ...
    var movementDelta = null;
    if (this._movement_delta) {
        movementDelta = {
    ...
    if (this._lastHit) {
        hitPoint = {
            x: this._lastHit.point.x,
            y: this._lastHit.point.y,
            z: this._lastHit.point.z
        };
        hitNormal = {
            x: this._lastHit.normal.x,
            y: this._lastHit.normal.y,
            z: this._lastHit.normal.z
        };
        hitDistance = this._lastHit.t;
    }

    return {
        movement: {
            raw_force: rawForce,
            projected_movement: projectedMovement,
