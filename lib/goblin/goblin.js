...
Goblin.NarrowPhase.prototype.generateContacts = function( possible_contacts ) {
    if (!Array.isArray(possible_contacts)) {
        console.warn("Invalid possible_contacts passed to generateContacts");
        return;
    }
    ...
    for (var i = 0; i < possible_contacts.length; i++) {
        // Validate the contact pair exists and has two objects
        if (!possible_contacts[i] || !Array.isArray(possible_contacts[i]) || 
            possible_contacts[i].length < 2 || 
            !possible_contacts[i][0] || !possible_contacts[i][1]) {
            console.warn("Invalid contact pair at index", i);
            continue;
        }

        try {
            var contact = this.getContact(possible_contacts[i][0], possible_contacts[i][1]);
            
            // Validate the contact before adding
            if (contact && contact.object_a && contact.object_b) {
                this.addContact(possible_contacts[i][0], possible_contacts[i][1], contact);
            }
        } catch (err) {
            console.warn("Error processing contact pair:", err);
            continue;
        }
    }
};
